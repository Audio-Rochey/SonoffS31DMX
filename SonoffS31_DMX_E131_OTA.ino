// NOT Working.
// Forked from SonoffS31_DMX_E131 to add the OTA.
// This is a sandbox.
...
// This is a fork of Cligerran example 4.
/*

Sonoff S31 is a smart power switch that has an ESP8266 inside. The same as as Wemos d1.
https://wiki.iteadstudio.com/images/f/ff/Sonoff-Schematic.pdf

The Switch on the Sonos is connected to ESP pin 15 / GPIO-0 / Wemos D3    https://www.espressif.com/sites/default/files/documentation/esp8266-technical_reference_en.pdf
The switched output is on PWM0, which is MTDI Pin 10 - GPIO 12 - Wemos D6
...
UPDATED 8/26/2024 to have mode data for use with ESPWebTools (e.g. how to config etc in the debug window on startup)
*/


// SONOFF S31 Pins Definition
#define ACRelay D6
#define SonoffButton D3
...
DirectONOFF LEDA(ACRelay, 0);

bool buttonPressed = false;
unsigned long lastDebounceTime = 0;
#define DEBOUNCE_DELAY 2000 // Milliseconds debounce delay


AsyncWebServer ohteeehserver(81);
...
void setup() {
  ...
  delay(500);
  Serial.println("==============================");
  Serial.println("Start Setup - Load or Generate Config");
   LoadConfig();
  //InitGpio();
  Serial.println("Init Wifi");
  InitWifi();
  Serial.println("Init 131");
  Init131();
  ...
  ohteeehserver.on("/", HTTP_GET, [](AsyncWebServerRequest *request) {
    request->send(200, "text/plain", "Hi! This is ElegantOTA AsyncDemo.");
  });

  ElegantOTA.begin(&ohteeehserver);    // Start ElegantOTA
  // ElegantOTA callbacks
  ElegantOTA.onStart(onOTAStart);
  ElegantOTA.onProgress(onOTAProgress);
  ElegantOTA.onEnd(onOTAEnd);

  ElegantOTA.setAuth("admin", "letmein");
...
void loop() {
  ...
  int buttonState = digitalRead(SonoffButton);
  if (buttonState == LOW) {  // Button is pressed (since using INPUT_PULLUP)
    unsigned long currentTime = millis();
    if (currentTime - lastDebounceTime > DEBOUNCE_DELAY) {
      // If it's a valid press, clear the Wi-Fi configuration
      if (!buttonPressed) {
        Serial.println("Button pressed! Clearing Wi-Fi configuration...");
        ClearWifiConfig();
        buttonPressed = true;  // Set the flag so that we don't repeatedly clear the config
      }
...
bool LoadConfig()
{
  ...
  if (!file) {
    ...
    Config["network"]["hostname"] = "Terrain-" + String(ESP.getChipId(), HEX);
...
void InitWifi()
{
  ...
  if(WiFi.waitForConnectResult(Timeout) != WL_CONNECTED) {
    ...
    Serial.println("");
    Serial.println("LOOKS LIKE I'M' CONNECTED TO A PROPER NETWORK! WELL DONE");
    Serial.printf("Open a browser to configure me at %s\n",WiFi.localIP().toString().c_str());
  }
...
void ClearWifiConfig() {
  digitalWrite(ACRelay, LOW); // Switch off the relay
  // Clear Wi-Fi configuration from LittleFS
  LittleFS.remove("/Config.json");  // Remove the configuration file
  
  // Optionally, reset Wi-Fi settings or restart the device
  WiFi.disconnect();
  delay(1000);  // Allow some time for Wi-Fi disconnect
  ESP.restart();  // Restart the ESP8266 to re-enter setup mode
}